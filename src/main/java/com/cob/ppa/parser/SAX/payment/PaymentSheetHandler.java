...
public class PaymentSheetHandler implements XSSFSheetXMLHandler.SheetContentsHandler {
    private final List<String> expectedHeaders = ExcelSheetHeaders.PAYMENT.getHeaders();
    private final Map<Integer, String> indexToHeader = new HashMap<>();
    private final List<PaymentDto> resultList = new ArrayList<>();
    private PaymentDto currentRow;
        ...
        if (currentRowNum == 0) {
            ...
            switch (header) {
                ...
                case "Date of Service":
                    currentRow.setDateOfService(DateParser.parse(formattedValue));
                    break;
                case "EMR Patient ID":
                    currentRow.setEmrPatientId(formattedValue);
                    break;
                case"Amount Paid":
                    currentRow.setAmountPaid(Double.parseDouble(formattedValue));
                    break;
            }
        }
    }
    ...
    public List<PaymentDto> getParsedResults() {
        return resultList;
    }
}
